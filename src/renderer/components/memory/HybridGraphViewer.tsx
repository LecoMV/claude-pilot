...
type ViewMode = 'auto' | 'cytoscape' | 'sigma'
...
interface HybridGraphViewerProps {
  className?: string
}

export function HybridGraphViewer({ className }: HybridGraphViewerProps) {
  ...
  // Determine which renderer to use based on node count
  const activeRenderer = useMemo(() => {
    if (viewMode !== 'auto') return viewMode
    if (!graphData) return 'cytoscape'
    return graphData.nodes.length > 1000 ? 'sigma' : 'cytoscape'
  }, [viewMode, graphData])
  ...
  // Cleanup function for both renderers
  ...
  const initSigma = useCallback(() => {
    ...
    const sigma = new Sigma(graph, containerRef.current, {
      ...
      defaultNodeColor: '#6c7086',
      defaultEdgeColor: '#3d3d5c',
      minCameraRatio: 0.1,
      maxCameraRatio: 10,
  ...
  const updateSigma = useCallback(() => {
    if (!graphologyRef.current || !sigmaRef.current || !graphData) return

    const graph = graphologyRef.current
    graph.clear()
    ...
    const fa2 = new FA2Layout(graph, {
      settings: {
        ...
        barnesHutOptimize: graphData.nodes.length > 2000,
        barnesHutTheta: 0.5,
      },
    })
  ...
  // Effect to manage renderer lifecycle
  useEffect(() => {
    cleanupRenderers()

    if (activeRenderer === 'cytoscape') {
      initCytoscape()
    } else if (activeRenderer === 'sigma') {
      initSigma()
    }
  ...
  }, [activeRenderer, cleanupRenderers, initCytoscape, initSigma])

  // Effect to update data when it changes
  useEffect(() => {
    if (!graphData) return

    if (activeRenderer === 'cytoscape' && cyRef.current) {
      updateCytoscape()
    } else if (activeRenderer === 'sigma' && sigmaRef.current) {
      updateSigma()
    }
  }, [graphData, activeRenderer, updateCytoscape, updateSigma])
    ...
    <div className={cn('h-full flex flex-col', className)}>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        >
          ...
          <option value={50000}>50K nodes</option>
        </select>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1">
          ...
          <div className="ml-4 flex items-center gap-2">
            ...
            >
              ...
              <option value="cytoscape">Cytoscape (&lt;1K)</option>
              <option value="sigma">Sigma.js (Large)</option>
            </select>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-text-muted">
          ...
          >
            {activeRenderer === 'cytoscape' ? 'Cytoscape' : 'Sigma.js'}
          </span>
          {graphData && (
            <span>
              ...
              {graphData.nodes.length >= nodeLimit && (
                <span className="text-accent-yellow ml-2">(limit reached)</span>
              )}
            </span>
          )}
        </div>
      ...
      {graphData && graphData.nodes.length > 10000 && (
        <div className="flex items-center gap-2 mb-2 p-2 bg-accent-yellow/10 border border-accent-yellow/30 rounded text-xs text-accent-yellow">
          <AlertTriangle className="w-4 h-4" />
          Large graph detected ({graphData.nodes.length.toLocaleString()} nodes). Performance may be
          impacted.
        </div>
      )}
